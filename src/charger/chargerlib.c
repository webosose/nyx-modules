...
nyx_error_t nyx_module_open(nyx_instance_t i, nyx_device_t **d)
{
	if (NULL == d)
	{
		nyx_error(MSGID_NYX_MOD_CHARG_OPEN_ERR, 0, "Charger device  open error.");
	...
	*d = NULL;
	...
	if (NULL == nyxDev)
	{
		nyx_error(MSGID_NYX_MOD_CHARG_OUT_OF_MEMORY, 0, "Out of memory");
	...
	nyx_error_t result = core_charger_init();
        ...
        nyx_charger_status_t *status)
{
	...
	return core_charger_read_status(status);
}
                                    ...
                                    nyx_charger_status_t *status)
{
	...
	return core_charger_enable_charging(status);
}
                                     ...
                                     nyx_charger_status_t *status)
{
	...
	return core_charger_disable_charging(status);
}
                                        ...
                                        nyx_charger_event_t *event)
{
	...
	return core_charger_query_charger_event(event);
}
